        ...
        if (schema instanceof Entity) {
            schema.denormalize = (input, unvisit) => {
                ...
                if (schema._cache[cacheKey] &&
                    ...
                    schema._nextCache[cacheKey] = schema._cache[cacheKey];
                    return schema._cache[cacheKey];
                }

                schema._nextCache[cacheKey] = denormalized;
                return denormalized;
            };
        }
            ...
            schema.denormalize = (input, unvisit) => {
                ...
                if (schema._cache[cacheKey] &&
                    Object.keys(schema._cache[cacheKey]).length === Object.keys(denormalized).length &&
                    Object.entries(denormalized).every(([key, value]) => value === schema._cache[cacheKey][key])) {
