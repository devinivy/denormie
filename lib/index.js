...
exports.createSelector = (getEntities, getResult, parentSchema, paths) => {
    ...
    let currentEntities = {};
    const schemas = [];
    const selectorizedSchemas = new Map();
    ...
    const cacheDenormalizeBy = ({ schema, getCacheKey, getIsEquivalent }) => {
        ...
        return (input, ...args) => {
            ...
            console.log({ result, cacheKey, cacheItem });
    ...
    const walkSchema = (schema) => {
        ...
        if (selectorizedSchemas.has(origSchema)) {
            return selectorizedSchemas.get(origSchema);
        }

        schema = (schema.denormalize && typeof schema.denormalize === 'function') ? clone(schema) :
            Array.isArray(schema) ? new ArraySchema(schema[0]) : new ObjectSchema(schema);
 
        selectorizedSchemas.set(origSchema, schema);
        schema._cache = null;
        schema._nextCache = {};
        schemas.push(schema);
        ...
        if (schema instanceof Entity) {
            schema.denormalize = cacheDenormalizeBy({
                ...
                getIsEquivalent: (result, cacheItem, cacheKey) => {
                    ...
                    console.log({
                        entitiesExist: !!lastEntities[schema.key] && !!currentEntities[schema.key],
                        entitiesIdentical: lastEntities[schema.key][cacheKey] === currentEntities[schema.key][cacheKey],
                        propsIdentical: Object.keys(schema.schema).every((relation) => {
                            
                            cacheItem[relation] !== result[relation] && console.log('bad', relation, cacheItem[relation], result[relation])

                            return cacheItem[relation] === result[relation];
                        })
                    });
