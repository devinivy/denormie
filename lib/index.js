...
exports.args = (parentSchema, entities, paths) => {
    ...
    const walkSchema = (schema, path = []) => {
        ...
        if (schema instanceof ArraySchema || schema instanceof UnionSchema || schema instanceof ValuesSchema) {
            ...
            Object.entries(schema.schema).forEach(([prop, subschema]) => {

                const nextPath = [...path, prop];

                if (paths.some((p) => startsWith(p, nextPath, { ignorePolymorphic: false }))) {
                    schema.schema[prop] = walkSchema(subschema, nextPath);
                }
                else {
                    delete schema.schema[prop];
                }
            });
        }
        else {
            ...
            Object.entries(schema.schema).forEach(([relation, subschema]) => {
                ...
                if (paths.some((p) => startsWith(p, nextPath, { ignorePolymorphic: true }))) {
